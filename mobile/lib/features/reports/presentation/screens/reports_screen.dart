        ...
        title: Row(
          ...
          children: [
            ...
            ShaderMask(
              ...
              shaderCallback: (b) =>
                  const LinearGradient(colors: kGradAmber).createShader(b),
              child: const Text(
                'Reports',
        ...
        bottom: PreferredSize(
          ...
          child: Column(
            ...
            children: [
              TabBar(
                controller:           _tabs,
                indicatorColor:       kGradAmber[0],
                labelColor:           kGradAmber[0],
      ...
      case 'Custom':
        final picked = await showDateRangePicker(
          ...
          initialDateRange: DateTimeRange(
              start: current.dateFrom, end: current.dateTo),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: selected
              ? const LinearGradient(colors: kGradAmber)
              : null,
          ...
          boxShadow: selected
              ? [
                  BoxShadow(
                    color:      kGradAmber[0].withValues(alpha: 0.25),
                    blurRadius: 8,
      ...
      child: ListView(
        ...
        children: [
          ...
          _GradSectionHeader(
            ...
            colors:   kGradSky,
            title:    'Daily Sales',
            trailing: dailyAsync.whenOrNull(
...
class _GradSummaryCard extends StatelessWidget {
  const _GradSummaryCard({
    required this.label,
    required this.value,
    required this.icon,
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          ...
                          belowBarData: BarAreaData(
                            show:  true,
                            color: kGradSky[0].withValues(alpha: 0.12),
                          ),
                        ),
                      ],
    ...
    return RefreshIndicator(
      ...
      child: productsAsync.when(
        ...
        data: (products) {
          ...
          final maxRevenue =
              products.map((p) => p.revenue).reduce(max);
...
class _ProductTile extends StatelessWidget {
  ...
  static const _rankGrads = [kGradAmber, kGradPrimary, kGradSky];

  @override
  Widget build(BuildContext context) {
    final cs       = Theme.of(context).colorScheme;
    final fraction = maxRevenue == 0 ? 0.0 : product.revenue / maxRevenue;
    final grad     = rank <= 3 ? _rankGrads[rank - 1] : kGradElectric;
...
class _GradSectionHeader extends StatelessWidget {
  const _GradSectionHeader({
    required this.icon,
    required this.colors,
    required this.title,
    ...
    return Container(
      ...
      child: Center(
        child: CircularProgressIndicator(
            color: kGradAmber[0], strokeWidth: 2),
      ),
    );
  }
}
