...
const PLANS = [
  ...
  {
    name: 'Professional',
    price: 2999,        // ← Change this value to update pricing
    period: 'month',
    currency: '₨',
  ...
  {
    name: 'Enterprise',
    price: 7999,        // ← Change this value to update pricing
    period: 'month',
    currency: '₨',
...
function FeatureValue({ val }) {
  ...
  if (val === false) return <span style={{ color: '#253550', fontSize: '1rem' }}>—</span>;
...
export default function PricingPage() {
  return (
    <div style={{ color: '#d1daf5' }}>
      ...
      {/* Hero */}
      <section className="pub-hero-bg" style={{ padding: '6rem 1.5rem 4rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <Fade>
            ...
            <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: '#f0f5ff', letterSpacing: '-0.03em', marginBottom: '1.25rem' }}>
              ...
              <span className="pub-gradient-text">for every business</span>
            </h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(290px, 1fr))', gap: '1.5rem', alignItems: 'start' }}>
            {PLANS.map((plan, i) => (
              <Fade key={plan.name} delay={i * 100}>
                <div style={{
                  background: plan.highlight ? 'linear-gradient(180deg, #0d1f40 0%, #0c1830 100%)' : '#0e1a30',
                  border: plan.highlight ? '1px solid rgba(59,130,246,0.4)' : '1px solid rgba(255,255,255,0.07)',
                  borderRadius: 20, padding: '2rem',
                  position: 'relative', overflow: 'hidden',
                  boxShadow: plan.highlight ? '0 0 0 1px rgba(59,130,246,0.15), 0 24px 64px rgba(0,0,0,0.4)' : '0 4px 24px rgba(0,0,0,0.2)',
                }}>
                  {plan.highlight && (
                    <div style={{
                  ...
                  {plan.badge && (
                    <span style={{
                      position: 'absolute', top: 16, right: 16, padding: '0.25rem 0.75rem',
                  ...
                  <div style={{ marginBottom: '1.5rem' }}>
                    <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#e8f0fe', marginBottom: '0.5rem' }}>{plan.name}</h3>
                    <p style={{ fontSize: '0.85rem', color: '#4a6080', lineHeight: 1.6 }}>{plan.desc}</p>
                  </div>

                  <div style={{ marginBottom: '2rem', paddingBottom: '2rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <span style={{ fontSize: 'clamp(2rem, 5vw, 2.75rem)', fontWeight: 900, color: '#f0f5ff', letterSpacing: '-0.04em' }}>
                      {formatPrice(plan)}
                    </span>
                    {plan.price > 0 && (
                      <span style={{ fontSize: '0.875rem', color: '#4a6080', marginLeft: '0.5rem' }}>/ {plan.period}</span>
                    )}
                  </div>
                  ...
                  <Link to="/login" style={{
                    ...
                    ...(plan.ctaStyle === 'gradient'
                      ...
                      : { background: 'rgba(59,130,246,0.08)', color: '#93c5fd', border: '1px solid rgba(59,130,246,0.25)' })
                  }}>
                    {plan.cta}
                  </Link>
                  ...
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                    {plan.features.map(f => (
                      <li key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.625rem', fontSize: '0.875rem', color: '#7a90b8' }}>
                        <CheckIcon style={{ width: 15, height: 15, color: plan.highlight ? '#3b82f6' : '#334e6e', flexShrink: 0, marginTop: 2 }} />
                        {f}
                      </li>
                    ))}
                  </ul>
      ...
      <section className="pub-section-alt" style={{ padding: '6rem 1.5rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <Fade style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <h2 style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2.25rem)', fontWeight: 800, color: '#f0f5ff', letterSpacing: '-0.03em' }}>
          ...
          <Fade>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: 'left', padding: '1rem 1.25rem', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#3d5070', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                      Feature
                    </th>
                    {PLANS.map(p => (
                      <th key={p.name} style={{ textAlign: 'center', padding: '1rem', fontSize: '0.875rem', fontWeight: 700, color: p.highlight ? '#60a5fa' : '#7a90b8', borderBottom: '1px solid rgba(255,255,255,0.07)', minWidth: 120 }}>
                        {p.name}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {COMPARE_FEATURES.map((row, i) => (
                    <tr key={row.feature} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)' }}>
                      <td style={{ padding: '0.875rem 1.25rem', fontSize: '0.875rem', color: '#7a90b8', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        {row.feature}
                      </td>
                      {[row.starter, row.pro, row.enterprise].map((val, j) => (
                        <td key={j} style={{ padding: '0.875rem', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
