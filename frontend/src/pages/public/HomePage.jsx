...
export default function HomePage() {
  ...
  return (
    <div style={{ color: '#d1daf5' }}>
      ...
      {/* ── Hero ──────────────────────────────────────────────────────────────── */}
      <section className="pub-hero-bg" style={{ padding: '7rem 1.5rem 6rem', minHeight: '92vh', display: 'flex', alignItems: 'center' }}>

        {/* Decorative blobs */}
        <div style={{
          position: 'absolute', top: 80, right: '8%', width: 500, height: 500, borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(6,182,212,0.08) 0%, transparent 70%)',
          pointerEvents: 'none', animation: 'pub-glow 12s ease-in-out infinite',
        }} />
        <div style={{
          position: 'absolute', bottom: '10%', left: '5%', width: 400, height: 400, borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(37,99,235,0.12) 0%, transparent 70%)',
          pointerEvents: 'none', animation: 'pub-glow 16s ease-in-out infinite 4s',
        }} />
          ...
          <div style={{ maxWidth: 760 }}>
            ...
            <div style={{ animation: 'pub-fade-up 0.6s ease both' }}>
              <span style={{
                ...
                fontSize: '0.8rem', fontWeight: 600, color: '#93c5fd', marginBottom: '1.75rem',
              }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b82f6', animation: 'pub-glow 2s ease-in-out infinite', display: 'inline-block' }} />
            ...
            {/* Headline */}
            <h1 style={{
              fontSize: 'clamp(2.5rem, 6vw, 4.25rem)', fontWeight: 900, lineHeight: 1.08,
              letterSpacing: '-0.03em', color: '#f0f5ff', marginBottom: '1.5rem',
              animation: 'pub-fade-up 0.6s ease 0.1s both',
            }}>
              Power Your Business<br />
              with <span className="pub-gradient-text">ProBusinessCloud</span>
            </h1>

            {/* Sub */}
            <p style={{
              fontSize: 'clamp(1rem, 2vw, 1.2rem)', lineHeight: 1.75, color: '#6b85aa',
              maxWidth: 600, marginBottom: '2.5rem',
              animation: 'pub-fade-up 0.6s ease 0.2s both',
            }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', animation: 'pub-fade-up 0.6s ease 0.3s both' }}>
              ...
              <Link to="/features" className="pub-btn-outline" style={{
                display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginTop: '3rem', flexWrap: 'wrap', animation: 'pub-fade-up 0.6s ease 0.4s both' }}>
              {['No Setup Fee', 'Cloud-Based', 'Multi-User', 'Secure & Reliable'].map(t => (
                <span key={t} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', color: '#3d5870' }}>
                  <CheckIcon style={{ width: 14, height: 14, color: '#06b6d4' }} />
                  {t}
                </span>
              ))}
            </div>
      ...
      {/* ── Stats bar ─────────────────────────────────────────────────────────── */}
      <section style={{ background: '#050912', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '2.5rem 1.5rem' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '2rem' }}>
          {STATS.map((s, i) => (
            <Fade key={s.label} delay={i * 80} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 900, letterSpacing: '-0.03em', color: '#f0f5ff', lineHeight: 1 }}>
                <span className="pub-gradient-text">{s.value}</span>
              </div>
              <div style={{ fontSize: '0.8rem', color: '#3d5070', marginTop: '0.5rem', fontWeight: 500 }}>{s.label}</div>
            </Fade>
          ))}
        </div>
      </section>
      ...
      <section className="pub-section" style={{ padding: '6rem 1.5rem' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <Fade style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.75rem)', fontWeight: 800, color: '#f0f5ff', letterSpacing: '-0.03em' }}>
              Built for every part of your business
            </h2>
            <p style={{ marginTop: '1rem', fontSize: '1rem', color: '#4a6080', maxWidth: 520, margin: '1rem auto 0' }}>
              ...
              return (
                <Fade key={f.title} delay={i * 60}>
                  <div className="pub-card" style={{ padding: '1.75rem' }}>
                    ...
                    <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#dce8ff', marginBottom: '0.5rem' }}>{f.title}</h3>
                    <p style={{ fontSize: '0.875rem', lineHeight: 1.65, color: '#4a6080' }}>{f.desc}</p>
                  </div>
                </Fade>
              );
            })}
              ...
              <div style={{
                ...
                <Fade style={{ flex: '1 1 340px' }}>
                  <span style={{
                    ...
                    background: `rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.12)`,
                    border: `1px solid rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.25)`,
                    fontSize: '0.78rem', fontWeight: 700, color: s.color, marginBottom: '1.25rem',
                  }}>
                    <Icon style={{ width: 14, height: 14 }} />
                    {s.label}
                  </span>
                  <h2 style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2.25rem)', fontWeight: 800, color: '#f0f5ff', letterSpacing: '-0.03em', marginBottom: '1rem' }}>
                    {s.title}
                  </h2>
                  <p style={{ fontSize: '1rem', lineHeight: 1.75, color: '#4a6080', marginBottom: '1.75rem' }}>{s.desc}</p>
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                    {s.points.map(pt => (
                      <li key={pt} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', fontSize: '0.9rem', color: '#7a90b8' }}>
                        <span style={{ width: 20, height: 20, borderRadius: '50%', background: `rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.15)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <Fade delay={120} style={{ flex: '1 1 320px' }}>
                  <div style={{
                    background: '#0c1628', border: `1px solid rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.15)`,
                    borderRadius: 20, padding: '2rem',
                    boxShadow: `0 32px 64px rgba(0,0,0,0.4), 0 0 80px rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.05)`,
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.15)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Icon style={{ width: 20, height: 20, color: s.color }} />
                      </div>
                      <span style={{ fontWeight: 700, color: '#ccd9f0', fontSize: '0.9rem' }}>{s.label}</span>
                    </div>
                    {s.points.map((pt, pi) => (
                      <div key={pi} style={{
                        display: 'flex', alignItems: 'center', gap: '0.75rem',
                        padding: '0.625rem', borderRadius: 10, marginBottom: '0.5rem',
                        background: pi === 0 ? `rgba(${s.color === '#3b82f6' ? '59,130,246' : s.color === '#06b6d4' ? '6,182,212' : s.color === '#8b5cf6' ? '139,92,246' : '16,185,129'},0.08)` : 'transparent',
                      }}>
                        <CheckIcon style={{ width: 14, height: 14, color: s.color, flexShrink: 0 }} />
                        <span style={{ fontSize: '0.825rem', color: pi === 0 ? '#bfcfe8' : '#4a6080' }}>{pt}</span>
                      </div>
                    ))}
                  </div>
                </Fade>
      ...
      {/* ── CTA Section ───────────────────────────────────────────────────────── */}
      <section style={{ padding: '7rem 1.5rem', background: '#050912', position: 'relative', overflow: 'hidden' }}>
        <div style={{
        ...
        <Fade style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center', position: 'relative' }}>
          <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 900, color: '#f0f5ff', letterSpacing: '-0.03em', marginBottom: '1.25rem' }}>
            Ready to transform<br />your business?
          </h2>
          <p style={{ fontSize: '1.05rem', lineHeight: 1.7, color: '#4a6080', marginBottom: '2.5rem' }}>
            Join hundreds of businesses already using ProBusinessCloud to streamline their operations and grow faster.
          </p>
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/login" className="pub-gradient-btn pub-btn-solid" style={{
              display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
              padding: '0.875rem 2.25rem', borderRadius: 14, fontWeight: 700,
              fontSize: '1rem', textDecoration: 'none',
              boxShadow: '0 8px 32px rgba(37,99,235,0.4)',
            }}>
              Get Started Free
            ...
            <Link to="/contact" className="pub-btn-outline" style={{
              display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
              padding: '0.875rem 2.25rem', borderRadius: 14, fontWeight: 600,
              fontSize: '1rem', textDecoration: 'none', color: '#93c5fd',
