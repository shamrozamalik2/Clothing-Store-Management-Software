...
const FAQS = [
  ...
  {
    q: 'Who can use ProBusinessCloud?',
  ...
  {
    q: 'Is ProBusinessCloud cloud-based?',
    a: 'Yes, ProBusinessCloud is fully cloud-based. Your data is stored securely in the cloud and can be accessed from any device with a browser — desktop, laptop, or tablet. All team members can work simultaneously in real-time, and your data is always synced across all sessions.',
  },
  {
    q: 'How does the login system work?',
    a: 'Each business gets a unique Company Code when they register. To sign in, users enter their Company Code, email address, and password. The system uses secure JWT-based authentication with automatic session refresh. Your company data is completely isolated from other businesses on the platform.',
  },
  {
  ...
  {
    q: 'Is the system suitable for small and large businesses?',
    a: 'Yes. ProBusinessCloud is designed to scale with your business. Small businesses can start with basic POS and inventory features, while larger businesses can activate advanced modules like HR, manufacturing, ledger management, and multi-user workflows. The pricing plans reflect different business sizes and needs, and you can upgrade as your business grows.',
  },
  {
    q: 'Can I manage multiple users on one account?',
    a: "Yes. You can add as many users as your plan allows. Each user gets their own login credentials and is assigned a role that controls their access level. You can activate, deactivate, or update users at any time. This makes it easy to onboard new staff and ensure they only see what's relevant to their job.",
  },
  {
    q: 'What payment methods does the POS support?',
    a: 'The POS system supports multiple payment methods including cash, card, bank transfer, and store credit. You can also process partial payments and track payment status on each sale. The system accommodates various checkout scenarios and keeps a complete record of every transaction.',
  },
  {
    q: 'Does ProBusinessCloud handle returns and exchanges?',
    a: "Yes. The system includes a complete Returns & Exchanges module. From a sale detail page, authorized users can process returns (with stock restored) or exchanges (where the customer swaps items). The system calculates the refund or additional payment amount and keeps a full audit trail of every return transaction.",
  },
  {
  ...
  return (
    <Fade delay={index * 40}>
      <div className="faq-item" style={{ padding: '0 0.25rem' }}>
        ...
        >
          <span style={{ fontSize: '1rem', fontWeight: 600, color: open ? '#93c5fd' : '#cdd9f0', lineHeight: 1.4, flex: 1 }}>
            {item.q}
          </span>
          <ChevronDownIcon style={{
            width: 18, height: 18, color: open ? '#3b82f6' : '#3d5070', flexShrink: 0,
        ...
        {open && (
          <div style={{ paddingBottom: '1.5rem', paddingRight: '2rem' }}>
            <p style={{ fontSize: '0.925rem', lineHeight: 1.8, color: '#5a7299' }}>{item.a}</p>
          </div>
        )}
      </div>
    </Fade>
...
export default function FAQPage() {
  return (
    <div style={{ color: '#d1daf5' }}>
      ...
      {/* Hero */}
      <section className="pub-hero-bg" style={{ padding: '6rem 1.5rem 4.5rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 620, margin: '0 auto' }}>
          <Fade>
            ...
            <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.25rem)', fontWeight: 900, color: '#f0f5ff', letterSpacing: '-0.03em', marginBottom: '1.25rem' }}>
              Everything you want to know<br />
              about <span className="pub-gradient-text">ProBusinessCloud</span>
            </h1>
      ...
      <section className="pub-section" style={{ padding: '4rem 1.5rem 6rem' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ background: '#0e1a30', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '1rem 2rem' }}>
            {FAQS.map((item, i) => (
      ...
      <section style={{ padding: '5rem 1.5rem', background: '#050912', textAlign: 'center' }}>
        <Fade style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          <Link to="/contact" className="pub-gradient-btn pub-btn-solid" style={{
