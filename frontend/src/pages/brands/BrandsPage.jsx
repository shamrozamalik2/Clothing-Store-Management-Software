...
export default function BrandsPage() {
  ...
  async function handleBulkDelete() {
    ...
    failed ? toast.error(`${failed} could not be deleted.`) : toast.success(`${ids.length} brand(s) deleted.`);
                ...
                <tr key={brand.id} className="hover:bg-surface-800/30 transition-colors">
                  <td className="px-4 py-3">
                    <input type="checkbox" className="h-4 w-4 rounded border-surface-600 bg-surface-700 text-primary-600 focus:ring-primary-500 cursor-pointer"
                      ...
                      onChange={e => { const n = new Set(selectedIds); e.target.checked ? n.add(brand.id) : n.delete(brand.id); setSelectedIds(n); }} />
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {brand.logo ? (
